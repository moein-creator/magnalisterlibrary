...
class eBayCategoryMatching {
	...
	private function rendereBayCategories($ParentID = 0, $purge = false) {
        ...
        if (    !$iStoreCategory
             && MLModul::gi()->getConfig('restrictedtobusiness') == '1') {
	...
	public function renderView() {
		if(MLModul::gi()->getConfig('restrictedtobusiness') == '1') {
...
(function($){
    ...
    function getEBayCategoryAttributes(cID,  sSelector, sMethod) {
        jqml('#'+sSelector).find('tr').not('.headline,.spacer').remove();
        jqml('#'+sSelector+' .headline').after('');
        jqml('#'+sSelector).css({'display':'none'});
        if (cID != 0) {
            mlShowLoading();
            jqml.ajax({
                type: 'POST',
                ...
                success: function(data) {
                    ...
                    jqml('#'+sSelector).find('tr').not('.headline,.spacer').remove();
                    jqml('#'+sSelector+' .headline').after(data+'');
                    if (data == '') {
                        jqml('#'+sSelector).css({'display':'none'});
                    } else {
                        jqml('#'+sSelector).css({'display':'table-row-group'});
                    }
                    mlHideLoading();
                },
                error: function() {
