    ...
    public function setOrder($sOrder){
        ...
        if(count($aOrder)==2){
            if($aOrder[0] == 'quantity'){
                ...
                $this->oSelect
                        ->join('(SELECT SUM(`instock`) quantity, `articleID` FROM `'.$sProductTableName.'` GROUP BY `articleID`) stock ON p.id = stock.articleid AND p.main_detail_id = details.id ', ML_Database_Model_Query_Select::JOIN_TYPE_INNER);
