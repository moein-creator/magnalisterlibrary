...
class ML_ShopwareCloud_Controller_Frontend_Do_ShopwareCloudDeleteShippingMethodCache extends ML_ShopwareCloud_Controller_Frontend_Do_ShopwareCloudAbstractCache {

    protected $sType = 'ShippingMethodDelete';
    protected $iLimitationOfIteration = 10;
    protected $sTranslationType = 'shipping-method';
    ...
    protected function getEntities($preparedFilters) {
        ...
        $shippingMethods = $this->shopwareEntityRequest->getShippingMethods('/api/search/' . $this->sTranslationType, 'POST', $preparedFilters);
        foreach ($shippingMethods->getData() as $shippingMethod) {
            $oTranslations = $this->shopwareEntityRequest->getShopwareTranslations('/api/' . $this->sTranslationType . '/' . $shippingMethod->getId() . '/translations');
