...
/**
 * This process get all available Customer Group from Shopware Cloud daily and update Customer Group data in magnalister
 * if some changes are missing in executeUpdate it will be applied at latest after a day
 */
class ML_ShopwareCloud_Controller_Frontend_Do_ShopwareCloudCustomerGroupCache extends ML_ShopwareCloud_Controller_Frontend_Do_ShopwareCloudAbstractCache {

    protected $sType = 'CustomerGroup';
    ...
    public function updateEntity($data) {
        $oTranslation = $this->shopwareEntityRequest->getShopwareTranslations('/api/customer-group/' . $data->getId() . '/translations');
        foreach ($oTranslation->getData() as $value) {
            MLDatabase::factory('ShopwarecloudTranslations')
                ...
                ->set('ShopwareTranslationType', 'customer-group')
    ...
    protected function getTotalCountOfEntities() {
        ...
        $entities = $this->shopwareEntityRequest->getShopwareCustomerGroup('/api/search/customer-group', 'POST', $preparedFilters);
        return $entities->getMeta()->getTotal();
    }

    protected function getEntities($preparedFilters) {
        return $this->shopwareEntityRequest->getShopwareCustomerGroup('/api/search/customer-group', 'POST', $preparedFilters);
    }
