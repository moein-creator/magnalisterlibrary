...
class ML_ShopwareCloud_Helper_Model_ProductList_Filter{
    protected $sPrefix='';
    protected $iPage=0;
    protected $iOffset=0;
    ...
    public function setOrder($sOrder){      
         ...
         if ( count($aOrder) == 2 && $aOrder[0] != '' && $aOrder[1] != '' ) {
            if ($aOrder[0] == 'manufacturerNumber') {
                 $aOrder[0] = 'p.ShopwareManufacturerNumber';
             }
             if ($aOrder[0] == 'ean') {
                 $aOrder[0] = 'p.ShopwareEAN';
             }elseif ($aOrder[0] == 'name' ) {
                $aOrder[0] = 't.`ShopwareName`';
                $this->oSelect->join(array('magnalister_shopwarecloud_product_translation', 't', 't.`ShopwareProductID` = p.`ProductsId` AND t.`ShopwareLanguageID`=\''.(string)$iLangId.'\''), 1);
            }elseif ($aOrder[0] == 'quantity' ) {                
                $aOrder[0] = 'p.`ShopwareQuantity`';
            }
     ...
     * @return ML_ShopwareCloud_Helper_Model_ProductList_Filter
     */
    public function registerDependency ($sDependency, $aDependecyConfig = array()) {
        ...
        if (!isset($this->aFilterOutput[$sName])) {
            ...
            if ($aIdentFilter['in'] !== null) {
                $sField = MLDatabase::factory('config')->set('mpid', 0)->set('mkey', 'general.keytype')->get('value') == 'pID' ? 'p.`ProductsId`' : 'p.`ProductsSku`';
