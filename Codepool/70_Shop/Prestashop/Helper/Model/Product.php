...
class ML_Prestashop_Helper_Model_Product {
    ...
    public function getProductByReference($sReference, $blOnlyMaster = false) {
        ...
        if ($iPID !== false) {
            $oProduct = MLProduct::factory()->loadPrestashopProduct($iPID, true);
        } elseif ($blOnlyMaster) {
            $oProduct = MLProduct::factory()->loadPrestashopProduct();
        } else {
    ...
    protected function getCombinationByReference($sReference) {
        ...
        if ($iPID !== false) {
            $oProduct = MLProduct::factory()->loadPrestashopVariation($iPID);
            return $oProduct;
        } else {
            return null;
        }
    ...
    public function getProductSelectQuery($blJustId = false) {
        MLProduct::factory()->initShopContext();
