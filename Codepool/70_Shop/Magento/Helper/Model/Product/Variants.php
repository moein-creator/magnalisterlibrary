...
class ML_Magento_Helper_Model_Product_Variants {
    ...
    public function getVariantCount() {
        ...
        if (in_array($this->oProduct->getTypeId(), array('simple', 'virtual'))) {
            ...
            // equal to $aChilds = $this->oProduct->getTypeInstance()->getUsedProducts() but load for defined shop
            /**
             * @var $oCollection Mage_Core_Model_Resource_Db_Collection_Abstract
             */
            $oCollection = $this
            ...
            $iCountOfChildren = $oCollection->count();
            if($iCountOfChildren === 0){
                MLMessage::gi()->addDebug('You are trying to show variation from a configurable product, but according to this query, it has no variation,
                 A probable reason could be that you didn\'t assign the product to the a website of your Magento-shop. To fix it, go to the product edit page in Magento admin, 
                 select "Website" from the right menu, choose the corresponding website and save the product, then you can see variations of the product.
                 Do the same change for all associated products');
            }
            $iOut = $iOptions * $iCountOfChildren;
        } else {
            $this->addMessage(
                MLI18n::gi()->get(
