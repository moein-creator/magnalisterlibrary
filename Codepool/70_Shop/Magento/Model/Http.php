...
class ML_Magento_Model_Http extends ML_Shop_Model_Http_Abstract {
    ...
    protected function getMagentoHttpClassType($sType) {
        if ($this->aHttpArray === null) {
            ...
            $this->aHttpArray = array(
                ...
                'sFrontendDoUrl' => $oShop->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK,
                        (
                            Mage::getStoreConfig(Mage_Core_Model_Store::XML_PATH_SECURE_IN_FRONTEND)
                            || Mage::getStoreConfig(Mage_Core_Model_Store::XML_PATH_SECURE_IN_ADMINHTML)
                        )
        ...
        // Always reinitialize request related data, no need to cache them, some methods might change request values
        $this->aHttpArray['aServerRequest'] = Mage::app()->getRequest()->getServer();
        $this->aHttpArray['aRequest'] = MLHelper::getArrayInstance()->mergeDistinct(Mage::app()->getRequest()->getParams(), Mage::app()->getRequest()->getPost());
    ...
    public function getShopImageUrl() {
        return  $this->getMagentoHttpClassType('sBaseUrl') . 'media/';
    }
