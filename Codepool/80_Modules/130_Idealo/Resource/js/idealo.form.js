            ...
            directBuyValue.on('change', function() {
                ...
                if (
                    ...
                    directBuyInputColumns.each(function() {
                        if (jqml(this).find('.ml-wrapped').length === 0) { // build wrapper element if not exists, only block makes problems with td
                    ...
                    if (jqml(".magnalisterForm td.input").closest("tr.js-field").not(":hidden").length <= directBuyRows.length) {
                        showDirectBuyPopup();
                    }
                }
            }).trigger('change');
