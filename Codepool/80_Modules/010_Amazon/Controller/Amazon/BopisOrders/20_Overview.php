...
class ML_Amazon_Controller_Amazon_BopisOrders_Overview extends ML_Core_Controller_Abstract {
    ...
    public static function getTabTitle() {
        return MLI18n::gi()->get('ML_AMAZON_BOPIS_ORDERS_OVERVIEW');
    }
    ...
    protected function setFilter() {
        ...
        $aSessionFilter = MLSession::gi()->get('AMAZONBOPISORDERLIST__filter.json');
        ...
        MLMessage::gi()->addDebug('filters', $aFilters);
        MLMessage::gi()->addDebug('$aRequestFilter', $aRequestFilter);

        MLSession::gi()->set('AMAZONBOPISORDERLIST__filter.json', $aFilters);
    ...
    protected function getOrderlist() {
        if ($this->oList === null) {
            $this->oList = ML::gi()->instance('model_list_amazon_bopisorder');
    ...
    public function getOrderListWidget() {
        ...
        $this->includeView('widget_list_bopisOrder', array('oList' => $oList, 'aStatistic' => $aStatistic));
    }
     ...
     * @return \ML_Amazon_Controller_Amazon_BopisOrders_Overview
     */
     ...
     * @return \ML_Amazon_Controller_Amazon_BopisOrders_Upload_Orderlist
     */
     ...
     * @return void
     */
    protected function callAjaxCancelOrRefundOrder() {
        $action = MLRequest::gi()->get('BopisAction') === 'cancel' ? 'AcknowledgeOrder' : 'InitiateRefund';
        try {
            $orderId = MLRequest::gi()->get('AmazonOrderID');
            if (empty($orderId)) {
                throw new MagnaException('AmazonOrderID can\'t be empty');
            }
            MagnaConnector::gi()->submitRequest(array(
                'ACTION' => $action,
                'DATA' => array(
                    'AmazonOrderIDs' => is_array(MLRequest::gi()->get('AmazonOrderID')) ? MLRequest::gi()->get('AmazonOrderID') : [MLRequest::gi()->get('AmazonOrderID')],
                    'AmazonOrderDetails' => is_array(MLRequest::gi()->get('AmazonOrderDetails')) ? MLRequest::gi()->get('AmazonOrderDetails') : [MLRequest::gi()->get('AmazonOrderDetails')],
                    'AdjustmentReasonCode' => MLRequest::gi()->get('AdjustmentReasonCode'),
                )
            ));
            MLSetting::gi()->add(
                'aAjax', array(
     ...
     * @return void
     */
    protected function callAjaxUpdateShippingStatus() {
        ...
        try {
            ...
            MagnaConnector::gi()->submitRequest(array(
                ...
                'DATA' => array(
                    'AmazonOrderIDs' => is_array(MLRequest::gi()->get('AmazonOrderID')) ? MLRequest::gi()->get('AmazonOrderID') : [MLRequest::gi()->get('AmazonOrderID')],
                    'AmazonOrderDetails' => is_array(MLRequest::gi()->get('AmazonOrderDetails')) ? MLRequest::gi()->get('AmazonOrderDetails') : [MLRequest::gi()->get('AmazonOrderDetails')],
                    'ShipmentStatus' => MLRequest::gi()->get('ShipmentStatus'),
                )
            ));
            MLSetting::gi()->add(
                'aAjax', array(
