...
class ML_Dawanda_Helper_Model_Service_Product {
    ...
    protected function getDescriptions() {
        ...
        foreach (MLModule::gi()->getConfig('langs') as $sIso => $iLangId) {
            if (!empty($iLangId)) {
        ...
        foreach ($aCatAttributes as $key => &$aCatAttribute) {
            ...
            if ($sCode === 'freetext' || $sCode === 'attribute_value') {
                ...
                if (empty($shopAttributeValue)) {
                    $shopAttributeValue = $this->oProduct->getAttributeValue($sCode);
                }
                ...
                if (empty($shopAttributeValue)) {
                    ...
                    if (isset($aCatAttribute['Values']) && is_array($aCatAttribute['Values'])) {
                        foreach ($aCatAttribute['Values'] as $value) {
                            if ($shopAttributeValue === $value['Shop']['Value'] 
                                || $shopAttributeValue === $value['Shop']['Key']) {
                                $aCatAttribute = $value['Marketplace']['Key'];
                                break;
                            }
                        }
                    } else {
